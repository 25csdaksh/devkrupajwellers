...
const SilverCollection = () => {
  ...
  const categories = ['All', 'Silver Payal', 'Silver Kamarband', 'Pooja Items', 'Silver Bracelet', 'Silver Rings'];

  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, 'silver_products'));
        ...
        if (productList.length > 0) {
          ...
          throw new Error("No silver products in DB, using fallback");
        }
      } catch (error) {
        console.log("Firebase empty or not configured for silver, using fallback");
    ...
    <div className="min-h-screen py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-5xl font-serif font-bold mb-4 text-primary">{t('silverCollection')}</h1>
          <p className="text-[var(--color-text-secondary)] opacity-90 max-w-2xl mx-auto">
            Browse through our premium, high-purity 925 sterling silver jewelry and sacred silver pooja articles.
          </p>
        </div>
        ...
        <div className="w-full">
          {loading ? (
            ...
            <div className="text-center py-12 text-muted bg-secondary/10 rounded border border-white/5">
              No silver items found in this category.
            </div>
          ) : (
              ...
              {filteredProducts.map((product) => (
                <Link to={`/product/${product.id}`} key={product.id} className="group glass-effect rounded-lg overflow-hidden border border-white/5 hover:border-accent/50 transition-all duration-300 flex flex-col h-full">
                  <div className="relative aspect-[4/5] bg-secondary/30 overflow-hidden flex items-center justify-center p-4">
                    ...
                    />
                    <div className="absolute top-4 right-4 bg-dominant/90 backdrop-blur px-3 py-1 text-xs font-semibold rounded-full border border-white/10 text-primary z-20">
                      {product.category}
                    </div>
                  </div>
                  <div className="p-5 flex-grow flex flex-col justify-between">
                    ...
                    <div className="flex justify-end items-center mt-auto pt-4 border-t border-white/5">
                      <span className="text-xs text-muted border border-white/10 rounded px-2 py-0.5 bg-white/5">92.5 Sterling</span>
                    </div>
                  </div>
                </Link>
              ))}
